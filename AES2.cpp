/* AES Encryption
   by Eric Bakan
   12/12/10
*/

#include "AES.h"
#include <iostream>

//Rijndael Rcon function
const uint8_t AES::rcon[255] = {
    };

//Rijndael S-Box
const uint8_t AES::sbox[256] =  {
    };

//Rijndael inverse S-box
const uint8_t AES::inv_sbox[256] = {
    };

//Rijndael multiplication by 2
const uint8_t AES::mult_2[256] = {
    };

//Rijndael multiplication by 3
const uint8_t AES::mult_3[256] = {
    };

//Rijndael multiplication by 9
const uint8_t AES::mult_9[256] = {
    };

//Rijndael multiplication by 11
const uint8_t AES::mult_11[256] = {
    };

//Rijndael multiplication by 13
const uint8_t AES::mult_13[256] = {
    };

//Rijndael multiplication by 14
const uint8_t AES::mult_14[256] = {
    };

//Rotates 4-byte array n steps to the left
void AES::rotate(uint8_t* word) {
    uint8_t t=word[0];
    for(int i=0;i<3;i++)
        word[i]=word[i+1];
    word[3]=t;
}

//Core function of Rjindael's Key Schedule
void AES::scheduleCore(uint8_t* word, int r) {
    rotate(word);

    for(int i=0;i<4;i++)
        word[i]=sbox[word[i]];

    word[0]^=rcon[r];
}

//Expands key into appropriate length for encryption level
uint8_t* AES::expandKey(int keySize, uint8_t* key) {

    int n=keySize/8; //number of bytes in key (16 in 128-bit encryption, 24 in 192-bit encryption, 32 in 240-bit encryption
    int b=176+(keySize-128)/2; //172-bit expanded key for 128-bit encryption, 208-bit expanded key for 192-bit encryption, 240-bit expanded key for 256-bit encryption
    uint8_t* expanded=new uint8_t[b];

    for(int i=0;i<n;i++) //first n bytes of expanded key are equal to key
        expanded[i]=key[i];

    int r=1; //rcon value
    int c=n; //start after initial equal bytes
    uint8_t t[4]; //temp array

    while(c<b) {
        for(int i=0;i<4;i++) //copy next set into temp array
            t[i]=expanded[c-4+i];

        if(!(c%n)) //every n bytes perform scheduleCore on temp array and increase rcon value
            scheduleCore(t,r++);

        if(keySize==256 && c%32==16) //special additional S-Box substitution in 256-bit encryption
            for(int i=0;i<4;i++)
                t[i]=sbox[t[i]];

        for(int i=0;i<4;i++) {
            expanded[c]=t[i]^expanded[c-n];
            c++;
        }
    }
    return expanded;
}

//XORs each byte of the state with its corresponding byte in the round key
void AES::addRoundKey(uint8_t* state, uint8_t* key) {
    for(int i=0;i<16;i++)
        state[i]^=key[i];
}

//Substitutes each byte with its corresponding byte in the S-Box
void AES::subBytes(uint8_t* state) {
    for(int i=0;i<16;i++)
        state[i]=sbox[state[i]];
}

//Shifts each row to the left corresponding to its row number
void AES::shiftRows(uint8_t* state) {
    uint8_t t[4];
    for(int i=1;i<4;i++) {
        for(int j=0;j<4;j++)
            t[j]=state[i+4*j];
        for(int j=0;j<4;j++)
            state[i+4*j]=t[(j+i)%4];
    }

}

//Multiplies each column by the MixColumns matrix
void AES::mixColumns(uint8_t* state) {
    uint8_t t[4];
    for(int i=0;i<4;i++) {
        for(int j=0;j<4;j++)
            t[j]=state[4*i+j];
        for(int j=0;j<4;j++)
            state[4*i+j]=mult_2[t[j]]^t[(j+3)%4]^t[(j+2)%4]^mult_3[t[(j+1)%4]];
    }
}

//Substitutes each byte with its corresponding byte in the inverse S-Box
void AES::inv_subBytes(uint8_t* state) {
    for(int i=0;i<16;i++)
        state[i]=inv_sbox[state[i]];
}

//Shifts each row to the right corresponding to its row number
void AES::inv_shiftRows(uint8_t* state) {
    uint8_t t[4];
    for(int i=1;i<4;i++) {
        for(int j=0;j<4;j++)
            t[j]=state[i+4*j];
        for(int j=0;j<4;j++)
            state[i+4*j]=t[((((j-i)%4)+4)%4)];
    }
}

//Multiplies each column by the inverse MixColumns matrix
void AES::inv_mixColumns(uint8_t* state) {
    uint8_t t[4];
    for(int i=0;i<4;i++) {
        for(int j=0;j<4;j++)
            t[j]=state[i*4+j];
        for(int j=0;j<4;j++)
            state[4*i+j]=mult_14[t[j]]^mult_9[t[(j+3)%4]]^mult_13[t[(j+2)%4]]^mult_11[t[(j+1)%4]];
    }
        
}

//Encrypts one 16-byte array of data
void AES::encryptBlock(int keySize, uint8_t* state, uint8_t* key) {
    int rounds=6+keySize/32;

    addRoundKey(state,key);

    for(int i=1;i<=rounds;i++) {

        subBytes(state);
        shiftRows(state);
        if(i!=rounds) //don't mix columns on last round
            mixColumns(state);

        addRoundKey(state,key+16*i);
    }
}

//Decrypts one 16-byte array of data
void AES::decryptBlock(int keySize, uint8_t* state, uint8_t* key) {
    int rounds=6+keySize/32;

    addRoundKey(state,key+rounds*16);

    for(int i=rounds-1;i>=0;i--) {

        inv_shiftRows(state);
        inv_subBytes(state);
        addRoundKey(state,key+16*i);
        if(i!=0) //don't mix columns on last round
            inv_mixColumns(state);
    }
}

//Encrypts plaintext with AES cipher
void AES::encrypt(int keySize, uint64_t numBytes, uint8_t* iv, uint8_t*& data, uint8_t* key) {
    int size=numBytes+16; //input array length + iv length
    if(numBytes%16!=0)
        size+=16-numBytes%16; //pads up to block size
    else
        size+=16; //else pads another block

    uint8_t* encrypted=new uint8_t[size]; //output array

    for(int i=0;i<16;i++) //copy over iv
        encrypted[i]=iv[i];

    uint8_t pnum=1;
    for(int i=16;i<size;i++) { //copy over array, pad if necessary
        if(i<(int) numBytes)
            encrypted[i]=data[i];
        else
            encrypted[i]=pnum++;
    }

    data=encrypted;

    uint8_t* expandedKey=expandKey(keySize,key); //generate expanded key

    for(int i=0;i<size/16;i++) { //encrypt each block
        encryptBlock(keySize,data+i*16,expandedKey);
    }
    
}

//Decrypts cipher with AES cipher
void AES::decrypt(int keySize, uint64_t numBytes, uint8_t*& data, uint8_t* key) {
    int size=numBytes-16; //don't worry about iv

    uint8_t* decrypted=new uint8_t[size];

    for(int i=0;i<size;i++) //copy over data
        decrypted[i]=data[i+16];

    uint8_t* expandedKey=expandKey(keySize,key); //generate expanded key
    for(int i=0;i<(int) numBytes/16;i++) { //decrypt each block
        decryptBlock(keySize,decrypted+i*16,expandedKey);
    }

    /*
    int padding=decrypted[size-1]; //len of padding

    uint8_t* unpadded=new uint8_t[numBytes-padding];
    for(int i=0;i<(int)numBytes-padding;i++)
        unpadded[i]=decrypted[i];

    delete decrypted;

    data=unpadded;*/
    data=decrypted;
}

//Encrypts stream with AES cipher
void AES::encryptStream (int keySize, uint8_t* iv, std::istream* in, std::ostream* out, uint8_t* key) {
    uint8_t* expandedKey=expandKey(keySize,key); //generate expanded key

    uint8_t newiv[16]; //temp iv to encrypt
    for(int i=0;i<16;i++)
        newiv[i]=iv[i];
    encryptBlock(keySize,newiv,expandedKey);
    (*out).write((char*)newiv,16);

    uint8_t block[16];
    uint8_t pnum=1; //padding counter
    char c;
    while(!in->eof()) {
        for(int i=0;i<16;i++) { //read block
            c=in->get(); //check if next char is eof
            if(!in->eof())
                block[i]=(uint8_t) c;
            else  //pad with chars
                block[i]=pnum++;
        }
        encryptBlock(keySize,block,expandedKey);
        out->write((char*)block,16);
    }
    if(pnum==1) {
        for(int i=0;i<16;i++)
            block[i]=(uint8_t)i;
        encryptBlock(keySize,block,expandedKey);
        out->write((char*)block,16);
    }
}

//Decrypts stream with AES cipher
void AES::decryptStream (int keySize, std::istream* in, std::ostream* out, uint8_t* key) {
    uint8_t* expandedKey=expandKey(keySize,key); //generate expanded key
    uint8_t block[16];
    uint64_t size;

    //get size of input data
    in->seekg(0,std::ios::end);
    size=in->tellg();
    in->seekg(0,std::ios::beg);
    std::cout << size << std::endl;
    std::cout << size/16 << std::endl;

    in->read((char*)block,16); //skips first 16 bytes of iv

    for(int i=1;i<(int)size/16;i++) {
        in->read((char*)block,16);
        decryptBlock(keySize,block,expandedKey);
        //std::cout << i << std::endl;
        if(i<(int)size/16-1) {//if not last block
            out->write((char*)block,16);
            //std::cout << (char*)block;
        }
        else {//depad
            //std::cout << "PADDING TIEM" <<std::endl;
            out->write((char*)block,16-block[15]);
            //std::cout << (char*)block;
        }
    }
}
